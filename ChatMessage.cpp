...
std::vector<char> ChatMessage::encodeMessage()
{
    ...
    message_composed += MESSAGE_FIELD_SEPARATOR;
    for(auto i : receivers)
    {
        message_composed += i;
        message_composed += MESSAGE_FIELD_SEPARATOR;
...
bool ChatMessage::decodeBody(std::vector<char> _body)
{
    ...
    try
    {
        ...
        temp_sender = temp.substr(0, temp.find(MESSAGE_FIELD_SEPARATOR));
        temp.erase(0, temp.find(MESSAGE_FIELD_SEPARATOR) + 1);

        for(int i = 0; i < temp_recv_num; i++)
        {
            temp_receivers.push_back(temp.substr(0, temp.find(MESSAGE_FIELD_SEPARATOR)));
            temp.erase(0, temp.find(MESSAGE_FIELD_SEPARATOR) + 1);
        }

        temp_body = temp.substr(0, temp.find(MESSAGE_FIELD_SEPARATOR));
